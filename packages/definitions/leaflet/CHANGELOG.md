# @atcute/leaflet

## 2.0.18

### Patch Changes

- 9002f28: pull latest Leaflet lexicons
- Updated dependencies [0e68928]
- Updated dependencies [0e68928]
  - @atcute/lexicons@2.1.1

## 2.0.17

### Patch Changes

- 00c9fbe: pull latest Leaflet lexicons

## 2.0.16

### Patch Changes

- 3f27a31: pull latest Leaflet lexicons

## 2.0.15

### Patch Changes

- b98846f: pull latest Leaflet lexicons

## 2.0.14

### Patch Changes

- 6a08873: pull latest Leaflet lexicons
- Updated dependencies [a73fa3b]
  - @atcute/lexicons@2.1.0

## 2.0.13

### Patch Changes

- b7f0901: pull latest Leaflet lexicons

## 2.0.12

### Patch Changes

- a5a7b97: pull latest Leaflet lexicons
- Updated dependencies [4fc7223]
  - @atcute/atproto@4.0.4

## 2.0.11

### Patch Changes

- 0b040ce: pull latest Leaflet lexicons

## 2.0.10

### Patch Changes

- 0b75a7c: pull latest Leaflet lexicons
- Updated dependencies [9882a31]
- Updated dependencies [cb01440]
- Updated dependencies [96c679b]
- Updated dependencies [f31de6d]
- Updated dependencies [0e039ef]
  - @atcute/lexicons@2.0.3

## 2.0.9

### Patch Changes

- 31a3a0b: drop sourcemaps and raw source files from publish
- Updated dependencies [31a3a0b]
  - @atcute/atproto@4.0.3
  - @atcute/lexicons@2.0.2

## 2.0.8

### Patch Changes

- 87f47dd: pull latest Leaflet lexicons

## 2.0.7

### Patch Changes

- 538067d: pull latest Leaflet lexicons

## 2.0.6

### Patch Changes

- 31c7a20: pull latest Leaflet lexicons

## 2.0.5

### Patch Changes

- d499e6b: pull latest Leaflet lexicons

## 2.0.4

### Patch Changes

- d7d30e4: pull latest Leaflet lexicons

## 2.0.3

### Patch Changes

- a836d6a: declare `sideEffects: false` so bundlers can tree-shake unused schema modules pulled in
  through the barrel
- Updated dependencies [a836d6a]
  - @atcute/atproto@4.0.2

## 2.0.2

### Patch Changes

- 012ff07: pull latest Leaflet lexicons

## 2.0.1

### Patch Changes

- 4e3ee0b: pull latest Leaflet lexicons
- d5444e7: pull latest Leaflet lexicons

## 2.0.0

### Patch Changes

- Updated dependencies [d64ddf1]
  - @atcute/lexicons@2.0.0
  - @atcute/atproto@4.0.0

## 1.0.25

### Patch Changes

- d174298: mark some dependencies as peer dependencies to avoid breakages in the future.
- Updated dependencies [d174298]
- Updated dependencies [87c00bb]
- Updated dependencies [8b6c404]
  - @atcute/atproto@3.1.12
  - @atcute/lexicons@1.3.1

## 1.0.24

### Patch Changes

- 9d749bd: pull latest Leaflet lexicons

## 1.0.23

### Patch Changes

- 202d384: pull latest Leaflet lexicons
- 43b2d76: regenerate with blob constraints
- Updated dependencies [94065a1]
  - @atcute/lexicons@1.3.0

## 1.0.22

### Patch Changes

- 22cb3aa: pull latest Leaflet lexicons
- Updated dependencies [4b99ff8]
- Updated dependencies [2022754]
  - @atcute/atproto@3.1.11
  - @atcute/lexicons@1.2.10

## 1.0.21

### Patch Changes

- 9b9c856: pull latest Leaflet lexicons

## 1.0.20

### Patch Changes

- dca650b: pull latest Leaflet lexicons

## 1.0.19

### Patch Changes

- 57bac6f: pull latest Leaflet lexicons

## 1.0.18

### Patch Changes

- 02dbe16: pull latest Leaflet lexicons

## 1.0.17

### Patch Changes

- 40a9d9e: pull latest Leaflet lexicons
- Updated dependencies [e73fddf]
  - @atcute/lexicons@1.2.7

## 1.0.16

### Patch Changes

- ae2d5d3: pull latest Leaflet lexicons
- f4378ab: pull latest Leaflet lexicons

## 1.0.15

### Patch Changes

- cd65daa: pull latest Leaflet lexicons
- Updated dependencies [336313e]
  - @atcute/atproto@3.1.10

## 1.0.14

### Patch Changes

- ba09724: pull latest Leaflet lexicons

## 1.0.13

### Patch Changes

- 8ed5ee9: pull latest Leaflet lexicons
- 327be13: pull latest Leaflet lexicons

## 1.0.12

### Patch Changes

- 11b2218: pull latest Leaflet lexicons
- Updated dependencies [8b5d7e0]
  - @atcute/atproto@3.1.9

## 1.0.11

### Patch Changes

- 1429438: add `atcute:lexicons` metadata to package.json

  all lexicon definition packages now include the `atcute:lexicons` field with namespace mappings,
  enabling automatic import resolution when used with `@atcute/lex-cli`'s `imports` configuration.

- Updated dependencies [1429438]
  - @atcute/atproto@3.1.8

## 1.0.10

### Patch Changes

- 8f4bd4b: add JSDoc to object fields
- Updated dependencies [8f4bd4b]
  - @atcute/atproto@3.1.7

## 1.0.9

### Patch Changes

- b30da0e: add `declarationMap` to tsconfig
- Updated dependencies [17c6f4a]
- Updated dependencies [b30da0e]
  - @atcute/lexicons@1.2.2
  - @atcute/atproto@3.1.6

## 1.0.8

### Patch Changes

- 43b5f62: pull latest Leaflet lexicons

## 1.0.7

### Patch Changes

- e20324c: pull latest Leaflet lexicons
- Updated dependencies [5c34327]
  - @atcute/atproto@3.1.4

## 1.0.6

### Patch Changes

- 0e529b5: pull latest Leaflet lexicons
- Updated dependencies [c3e2999]
  - @atcute/atproto@3.1.3

## 1.0.5

### Patch Changes

- 66c7512: pull latest Leaflet lexicons

## 1.0.4

### Patch Changes

- 83de3cc: pull latest Leaflet lexicons
- Updated dependencies [394080c]
- Updated dependencies [a53bc05]
  - @atcute/lexicons@1.1.1
  - @atcute/atproto@3.1.2

## 1.0.3

### Patch Changes

- 9438aa8: pull latest Leaflet lexicons

## 1.0.2

### Patch Changes

- bf209f5: pull latest Leaflet lexicons

## 1.0.1

### Patch Changes

- cf9e31e: pull latest Leaflet lexicons
