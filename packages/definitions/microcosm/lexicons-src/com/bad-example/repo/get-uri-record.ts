...
export default document({
	...
	defs: {
		main: query({
			...
			parameters: params({
				properties: {
					at_uri: required(string({ format: 'at-uri', description: 'the at-uri of the record (identifier can be a DID or handle)' })),
					cid: string({ format: 'cid', description: 'optional CID of the version of the record. if not specified, return the most recent version. if specified and a newer version exists, returns 404.' }),
				},
			}),
			output: {
