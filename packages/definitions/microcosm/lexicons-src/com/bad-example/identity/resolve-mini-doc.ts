	...
	defs: {
		main: query({
			...
			output: {
				...
				schema: object({
					properties: {
						did: required(string({ format: 'did', description: 'DID, bi-directionally verified if a handle was provided in the query' })),
						handle: required(string({ format: 'handle', description: 'the validated handle of the account or \'handle.invalid\' if the handle did not bi-directionally match the DID document' })),
