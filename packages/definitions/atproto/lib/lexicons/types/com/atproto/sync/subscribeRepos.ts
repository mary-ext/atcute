...
const _commitSchema = /*#__PURE__*/ v.object({
	...
	/**
	 * CAR file containing relevant blocks, as a diff since the previous repo state. The commit must be included
	 * as a block, and the commit block CID must be the first entry in the CAR header 'roots' list.
	 *
	 * @maxLength 2000000
	 */
	...
	/**
	 * The root CID of the MST tree for the previous commit from this repo (indicated by the 'since' revision
	 * field in this message). Corresponds to the 'data' field in the repo commit object. NOTE: this field is
	...
	/**
	 * The rev of the emitted commit. Note that this information is also in the commit object included in
	...
	time: /*#__PURE__*/ v.datetimeString(),
	/**
	 * DEPRECATED -- replaced by #sync event and data limits. Indicates that this commit contained too many ops,
	 * or data size was too large. Consumers will need to make a separate request to get missing data.
	 */
...
const _identitySchema = /*#__PURE__*/ v.object({
	...
	did: /*#__PURE__*/ v.didString(),
	/**
	 * The current handle for the account, or 'handle.invalid' if validation fails. This field is optional,
	 * might have been validated or passed-through from an upstream source. Semantics and behaviors for PDS vs
...
const _repoOpSchema = /*#__PURE__*/ v.object({
	...
	/**
	 * For updates and deletes, the previous record CID (required for inductive firehose). For creations, field
...
const _syncSchema = /*#__PURE__*/ v.object({
	...
	/**
	 * CAR file containing the commit, as a block. The CAR header must include the commit block CID as the first
	 * 'root'.
	 *
	 * @maxLength 10000
	 */
