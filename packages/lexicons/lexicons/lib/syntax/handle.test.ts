...
describe('handle validation', () => {
	it('validates handle', () => {
		...
		for (const case_ of validCases) {
			expect(isHandle(case_), case_).toBe(true);
		}
		...
		const invalidCases = [
			...
			'shoooort' + '.loooooooooooooooooooooooooong'.repeat(9) + '.test',

			// max segment: 'short.' + 'o'.repeat(64) + '.test'
			'short.' + 'o'.repeat(64) + '.test',
		...
		for (const case_ of invalidCases) {
			expect(isHandle(case_), case_).toBe(false);
		}
	});
});
