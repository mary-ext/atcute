...
describe('readCarStream', () => {
	it('reads car files', async () => {
		...
		const stream = toStream(buf);

		const { roots, iterate } = await readCarStream(stream);
		const blocks = await toArray(iterate());
...
function toStream(buffer: Uint8Array, chunkSize = 128) {
	let offset = 0;
