...
the name that's provided to it, like so:
...
we'd then write a configuration file for the CLI tool to locate the lexicon documents and generate
the validation schemas that we need:
...
run the CLI tool pointing to the config file
...
on Deno, Bun or Cloudflare Workers, you can export the router directly and it will work out of the
box.
