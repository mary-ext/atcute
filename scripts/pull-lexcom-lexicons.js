...
const repo = `lexicon-community/lexicon`;
...
async function main() {
	...
	let sha;
	{
		...
		const response = await fetch(`https://api.github.com/repos/${repo}/commits?path=community/lexicon/`);
...

	{
		...
		const basename = `lexicon-${sha}/community/lexicon/`;
