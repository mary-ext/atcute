...
the result shape from `safeParse` is `{ success, output, issues }` rather than `{ ok, value,
issues }`, and issues no longer carry a `code` field.

every object in the schema graph is now a `looseObject`, so unknown fields are preserved by
default; this matches what every consumer in the workspace was already opting into via `mode:
'passthrough'`.
